        ...
        for(AnnotationEntry entry : annotationList) {
            ...
            switch(dbcatid) {
                case TRANSCRIPT:
                    ...
                    double wtxt =  wtxttext.getLayoutBounds().getWidth();
                        ...
                        default:
                            ...
                            if(!project.data.isInternalFeature(entry.source, entry.feature) || entry.feature.equals(DataAnnotation.INTCAT_FLUSHDATA)) {
                                ...
                                if(args.prefs.alignment) {
                                    ...
                                    if(!genomic) {  // temp?
                                        entry.dspstrpos = seqAlign.getAlignedPos(entry.strpos, tgexonsList, negStrand);
                                        entry.dspendpos = seqAlign.getAlignedPos(entry.endpos, tgexonsList, negStrand);
                                ...
                                genomic = (genomic || entry.source.equals("sQTL"));
                                if(genomic) {
