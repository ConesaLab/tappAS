 ...
 */
public class DrillDownUTRLClusters extends DlgBase {
    static int id = 0;
    ...
    int toolId;
    DataUTRL utrlData;
    String utrlDataTypeName;
    String grpName;
    ArrayList<Integer> lstClusters;
    String exportFilename;
    
    public DrillDownUTRLClusters(Project project, Window window) {
        super(project, window);
        toolId = ++id;
    }
    public void showAndWait(String utrlDataTypeName, String grpName, ArrayList<Integer> lstClusters) {
        if(createToolDialog("DrillDownUTRLClusters.fxml", "UTRL Gene Data Visualization",  null)) {
            this.utrlData = new DataUTRL(project);
            this.utrlDataTypeName = utrlDataTypeName;
            this.grpName = grpName;
            this.lstClusters = lstClusters;
            exportFilename = "tappAS_UTRL_" + utrlDataTypeName + " ExpProfileClusters.png";
            ...
            // setup dialog
            setProjectName();
            dialog.setTitle("UTRL " + utrlDataTypeName + " Expression Profile Clusters");
            ...
            for(int row = 0; row < nrows; row++) {
                for(int col = 0; col < ncols; col++) {
                    String filename = utrlData.getUTRLClusterImageFilepath(grpName, cnum++);
            ...
            app.ctls.setupArchorPaneExport(apClusters, "UTRL " + utrlDataTypeName + " Expression Profile Clusters", exportFilename, null);
            ...
            btnHelpTbl.setOnAction((event) -> { DlgHelp dlg = new DlgHelp(); dlg.show(title, "Help_DrillDown_UTRL_Clusters.html", Tappas.getWindow()); });
    ...
    protected void onButtonExportChart() {
        ...
        app.export.exportImage("UTRL " + utrlDataTypeName + " Expression Profile Clusters", exportFilename, img);
    }
}
