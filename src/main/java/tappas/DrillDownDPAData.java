 ...
 */
public class DrillDownDPAData extends DlgBase {
    static int id = 0;
    ...
    ObservableList<DPADrillDownData> data = null;
    
    public DrillDownDPAData(Project project, Window window) {
        super(project, window);
        toolId = ++id;
    }
    public HashMap<String, String> showAndWait(String gene, HashMap<String,ArrayList<String>> iso) {
        if(createToolDialog("DrillDownDPAData.fxml", "DPA Features Drill Down Data",  null)) {
            this.gene = gene;
            this.shortIso = iso.get("S");
            this.longIso = iso.get("L");
            panel = TabGeneDataViz.Panels.TRANS.name();
            idColName = "Gene";
            exportFilename = "tappAS_DPA_Isoforms_" + gene + ".tsv";
            // get control objects
            ...
            // setup dialog
            dialog.setTitle("DPA Drill Down Data for " + gene);
            ...
            btnHelpTbl.setOnAction((event) -> { DlgHelp dlg = new DlgHelp(); dlg.show(title, "Help_DrillDownData_DPA.html", Tappas.getWindow()); });
    ...
    private class DataService extends TaskHandler.ServiceExt {
        ...
        protected void onFailed() {
            ...
            if(e != null)
                app.logWarning("DrillDownDPA - task aborted. Exception: " + e.getMessage());
            else
                app.logWarning("DrillDownDPA - task aborted.");
            app.ctls.alertWarning("Drill Down", "DrillDownDPA - task aborted");
        }
        @Override
        protected void onSucceeded() {
            if(data != null) {
                ...
                app.ctls.alertWarning("DPA Drill Down Data", "Unable to load drill down data.");
        }
        @Override
        protected Task<Void> createTask() {
            Task task = new Task<Void>() {
                ...
                protected Void call() throws Exception {
                    try {
                        ObservableList<DPADrillDownData> lst = FXCollections.observableArrayList();
                        int max = Math.max(shortIso.size(),longIso.size());
                        for(int i=0;i<max;i++){
                            if(i<=shortIso.size()-1 && i <= longIso.size()-1)
                                lst.add(new DPADrillDownData(shortIso.get(i), longIso.get(i)));
                            else if(i>longIso.size()-1 && i <= shortIso.size()-1)
                                lst.add(new DPADrillDownData(shortIso.get(i), ""));
                            else
                                lst.add(new DPADrillDownData("", longIso.get(i)));  
                        }
            ...
            taskInfo = new TaskHandler.TaskInfo("DPA Drill Down Data " + toolId, task);
            return task;
        }
    }
    ...
    // Data Classes
    //
    static public class DPADrillDownData implements Comparable<DPADrillDownData> {
        //public SimpleStringProperty gene;
        public SimpleStringProperty shortIso;
        public SimpleStringProperty longIso;
        public DPADrillDownData(String shortIso, String longIso) {
        ...
        @Override
        public int compareTo(DPADrillDownData td) {
            return 1;
        }
        /*public int compareTo(DPADrillDownData td) {
