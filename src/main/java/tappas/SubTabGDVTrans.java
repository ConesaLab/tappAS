 ...
 */
public class SubTabGDVTrans extends SubTabBase {
    Canvas canvas;
    ScrollPane spCanvas;
    TabGeneDataViz tabDataViz;

    private final ViewTranscript dvView;

    // canvas settings - transcripts
    private final ViewAnnotation.Prefs dvPrefs = new ViewAnnotation.Prefs(true, 1.25, 1, false, true, true, false, false, false, ViewAnnotation.SortBy.ID);
    
    public SubTabGDVTrans(Project project) {
        super(project);
        dvView = new ViewTranscript(project);
    }
    @Override
    public boolean initialize(TabBase tabBase, SubTabInfo subTabInfo, HashMap<String, Object> args) {
        // setup subtab specific settings
    ...
    public Object processRequest(HashMap<String, Object> hm) {
        ...
        if(hm.containsKey("zoomIn")) {
            ...
            if(ppb < TabGeneDataViz.MAX_PPB && widthFactor <= TabGeneDataViz.MAX_WIDTHFACTOR)
                tabDataViz.updateWidth(dvView, dvPrefs, widthFactor);
            updateZoom();
        }
    ...
    private void exportImage() {
        ...
        app.export.exportImage("Gene Transcripts Visualization", "tappAS_"+tabDataViz.gene+"_Gene_Transcrips_Visualization.png", (Image)img);
        //app.export.setupCanvasExport(canvas, "Gene Transcripts Visualization", "tappas_Gene_Transcrips_Visualization.png");
    }
