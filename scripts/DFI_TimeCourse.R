...
Favored_tappas <- function(exp, design)
  ...
  LIST = NULL
  if(multiple){ #multiple time-series
    ...
    if(length(unique(genes[order(genes)]))==1){ #transform into data.frame with values per colum
      zzmMotif <- t(as.data.frame(zzmMotif))
      zzmNMotif <- t(as.data.frame(zzmNMotif))
    }
...
if(verbose){
  ...
  outputTestFeatures = "./"
  outputTestGenes = "./"
  analysisId = "01819246184"
  indirDFI = "./"
}

transFeatures <- c("NMD,CDS,5UTR_Length,polyA_Site,3UTR_Length,PAS,uORF,5UTRmotif,3UTRmotif,repeat,miRNA_Binding")
proteinFeatures <- c("SIGNAL,DOMAIN,ACT_SITE,INTRAMEM,TRANSMEM,PTM,BINDING,MOTIF,COILED,COMPBIAS")
