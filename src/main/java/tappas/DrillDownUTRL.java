 ...
 */
public class DrillDownUTRL extends DlgBase {
    static int id = 0;
    ...
    DataUTRL.UTRLSelectionResults data = null;
    
    public DrillDownUTRL(Project project, Window window) {
    ...
    public HashMap<String, String> showAndWait(DataUTRL.UTRLSelectionResults data) {
        if(createToolDialog("DrillDownUTRL.fxml", "UTRL Gene FeatureId Data Visualization",  null)) {
            this.data = data;
            ...
            for(int j = 0; j < num_groups; j++) {
                ...
                for(String name : names){ //by each group name - add point
                    if(j==0)
                        expValues[idx]=data.expr[idx];
                    else
                        expValues[idx]=data.expr[1+idx];
...
/*            chartCondition.getData().addAll(series3);
            ...
            app.ctls.setupChartExport(chartMain, "UTRL Gene Data Visualization", exportFilename, null);
            ...
            btnHelpTbl.setOnAction((event) -> { DlgHelp dlg = new DlgHelp(); dlg.show(title, "Help_DrillDown_UTRL.html", Tappas.getWindow()); });
    ...
    protected void onButtonExportChart() {
        ...
        app.export.exportImage("UTRL Gene FeatureId Data Visualization", exportFilename, img);
    }
}
