 ...
 */
public class SubTabDPAVennDiag extends SubTabBase {
    ...
    DataApp.DataType dpaType;
    String dpaTypeID, dpaTypeName;
    DataDPA dpaData;
    ...
    public SubTabDPAVennDiag(Project project) {
        super(project);
    }
    @Override
    public boolean initialize(TabBase tabBase, SubTabInfo subTabInfo, HashMap<String, Object> args) {
        ...
        if(result) {
            if(hasData) {
                ...
                app.ctls.alertInformation("DPA " + dpaTypeName + " Venn Diagram", "No DPA data available.");
        }
        if(!result)
...
/*        
        ...
        app.ctls.setupImageExport(imgView, "DPA Significant " + dpaTypeName + " Venn Diagram", "tappas_" + dpaTypeName + "_VennDiag.png", null);
        ...
        app.export.setupSimpleTableExport(tblMembers, cm, true, "tappAS_DPA_Significant" + dpaTypeName + ".tsv");
    ...
    private void setupExportMenu() {
        ContextMenu cm = new ContextMenu();
        MenuItem item = new MenuItem("Export DPA significant " + dpaTypeName + " table data...");
        item.setOnAction((event) -> { app.export.exportTableData(tblMembers, true, "tappAS_DPA_Significant_" + dpaTypeName + ".tsv"); });
        cm.getItems().add(item);
        cm.getItems().add(new SeparatorMenuItem());
        item = new MenuItem("Export Venn Diagram");
        item.setOnAction((event) -> { 
            app.export.exportImage("DPA Significant " + dpaTypeName + " Venn Diagram", "tappAS_" + dpaTypeName + "_VennDiag.png", imgView.getImage());
        });
        cm.getItems().add(item);
        cm.getItems().add(new SeparatorMenuItem());
        ...
        item.setOnAction((event) -> { 
            ...
            app.export.exportImage("Data Summary Panel", "tappAS_DPA_Significant_" + dpaTypeName + "_Panel.png", (Image)img);
        });
        cm.getItems().add(item);
    ...
    private class DataLoadService extends TaskHandler.ServiceExt {
        ...
        protected void onFailed() {
            ...
            if(e != null)
                app.logWarning("DPA VennDiag failed - task aborted. Exception: " + e.getMessage());
            else
                app.logWarning("DPA VennDiag - task aborted.");
            app.ctls.alertWarning("Differential Expression Analysis Venn Diagram", "DPA VennDiag failed - task aborted");
        }
        @Override
        ...
        protected Task<Void> createTask() {
            Task task = new Task<Void>() {
                ...
                protected Void call() throws Exception {
                    ...
                    lst.add("\"" + strNames + "\"");
                    lst.add(strCounts);
                    runScript(taskInfo, lst, "VennDiagram " + dpaTypeName, logfilepath);
            ...
            taskInfo = new TaskHandler.TaskInfo("Venn Diagram " + dpaTypeName, task);
            return task;
        }
    }
