 ...
 */
public class SubTabDataLengthPerSC extends SubTabBase {
    AnchorPane paneContents;
    BarChart barChart;
    
    public SubTabDataLengthPerSC(Project project) {
        super(project);
    }
    @Override
    ...
    private void setupExportMenu() {
        ...
        item.setOnAction((event) -> { 
            ...
            app.export.exportImage("Transcript Lengths Distribution per Structural Category", "tappAS_TransLengths_Distribution_PerStructCategory.png", (Image)img);
        });
        ...
        item.setOnAction((event) -> { 
            ...
            app.export.exportImage("Transcript Lengths Distribution per Structural Category", "tappAS_TransLength_Distribution_PerStructCategory_Panel.png", (Image)img);
        });
        cm.getItems().add(item);
        subTabInfo.cmExport = cm;
    } 
    private void showDistributionPerSC() {
        barChart.setTitle("Transcript Lengths Distribution per Structural Category");
        barChart.getXAxis().setLabel("Structural Category");
        barChart.getYAxis().setLabel("Length (NTs)");
        barChart.setBarGap(4.0);
        barChart.setCategoryGap(10.0);
        BoxPlotChart boxPlot = new BoxPlotChart();
        HashMap<String, ArrayList<Integer>> hmAlignCats = project.data.getAlignmentCategoriesTransLengths(project.data.getResultsTrans());
        ...
        app.ctls.setupChartExport(barChart, "Transcripts Lengths Distribution per Structural Category Chart", "tappAS_TransLengthDistributionStructCatChart.png", null);
