...
public class AnnotationSymbols extends AppObject {
    ...
    public boolean isSingleColumnLegend(GraphicsContext gc,Font font, Rectangle2D.Double rc) {
        ...
        if(cnt > 0) {
            ...
            for(SymbolType symtype : symbolTypesList) {
                ...
                Text widthtext = new Text(caption);
                widthtext.setFont(gc.getFont());
                width =  widthtext.getLayoutBounds().getWidth();
    ...
    public void drawSpliceJunction(GraphicsContext gc, double x, double y, double height, boolean novel, boolean sQTL, int drawType) {
        Color color;
        if(novel)
            color = Color.web("0xFF0000", 1);
        else if (sQTL)     
            color = Color.web("0x0004FF", 1);
        else
