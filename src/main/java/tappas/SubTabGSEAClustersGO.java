 ...
 */
public class SubTabGSEAClustersGO extends SubTabBase {
    ...
    public SubTabGSEAClustersGO(Project project) {
        super(project);
    }
    @Override
    public boolean initialize(TabBase tabBase, SubTabInfo subTabInfo, HashMap<String, Object> args) {
        ...
        if(result) {
            feaData = new DataGSEA(project);
            if(!args.containsKey("id")) {
                if(subTabInfo.subTabId.length() > TabProjectDataViz.Panels.CLUSTERSGOGSEA.name().length())
                    args.put("id", subTabInfo.subTabId.substring(TabProjectDataViz.Panels.CLUSTERSGOGSEA.name().length()).trim());
            }
            if(args.containsKey("id") && args.containsKey("lstClusterTerms")) {
                ...
                feaParams = feaData.getGSEAParams(analysisId);
                params = new DlgGSEAnalysis.Params(feaParams);
                result = feaData.hasGSEAData(params.dataType.name(), analysisId);
                if(result) {
                    String shortName = getShortName(params.name);
                    subTabInfo.title = "GSEA Clusters GO: '" + shortName + "'";
                    subTabInfo.tooltip = "GSEA enriched features clusters GO Terms for '" + params.name + "'";
    ...
    private void exportImage() {
        ...
        app.export.exportImage("GSEA Cluster Gene Ontology Visualization", "tappAS_GSEA_"+params.name+"_Cluster_Gene_Ontology_Visualization.png", (Image)img);
    }
