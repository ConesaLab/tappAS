 ...
 */
public class SubTabGSEAEnriched extends SubTabBase {
    ...
    String analysisId = "";
    ArrayList<ArrayList<DataGSEA.GSEAResultsData>> lstChartData = null;
    DlgGSEAFilterFeatures.Params filterParams = null;
    HashMap<String, HashMap<String, Object>> hmFeatureIDs = new HashMap<>();
    
    public SubTabGSEAEnriched(Project project) {
        super(project);
    }
    @Override
    public boolean initialize(TabBase tabBase, SubTabInfo subTabInfo, HashMap<String, Object> args) {
        ...
        if(result) {
            ...
            if(!args.containsKey("id")) {
                if(subTabInfo.subTabId.length() > TabProjectDataViz.Panels.ENRICHEDGSEA.name().length())
                    args.put("id", subTabInfo.subTabId.substring(TabProjectDataViz.Panels.ENRICHEDGSEA.name().length()).trim());
            }
            if(args.containsKey("id")) {
                ...
                subTabInfo.title = "GSEA Enriched: " + shortName;
                subTabInfo.tooltip = "GSEA enriched gseatures for '" + gseaParams.name.trim() + "'";
                setTabLabel(subTabInfo.title, subTabInfo.tooltip);
                filterParams = new DlgGSEAFilterFeatures.Params(gseaParams.getFeaturesName());
            }
            else
                result = false;
            if(!result)
    ...
    private void showSubTab() {
        ...
        showData();
        app.ctls.setupChartExport(scatChart, "GSEA Enriched Features", "tappAS_GSEA_"+gseaParams.name.trim()+"_Enriched_Features_Chart.png", null);
    ...
    private void setupExportMenu() {
        ...
        item.setOnAction((event) -> { 
            ...
            app.export.exportImage("GSEA Enriched Features", "tappAS_GSEA_"+gseaParams.name.trim()+"_Enriched_Features.png", (Image)img);
        });
        ...
        item.setOnAction((event) -> { 
            ...
            app.export.exportImage("GSEA Enriched Features", "tappAS_GSEA_"+gseaParams.name.trim()+"_Enriched_Features_Panel.png", (Image)img);
        });
        cm.getItems().add(item);
    ...
    private void showData() {
        ...
        if(filterParams.type.equals(DlgGSEAFilterFeatures.Params.Type.TOPN))
            title = "Top " + filterParams.n + " Feature IDs for each Feature";
        else if(filterParams.type.equals(DlgGSEAFilterFeatures.Params.Type.MANUAL)) {
            topn = false;
            manual = true;
            ...
            lstChartData = gseaData.getGSEAChartData(gseaParams.dataType.name(), analysisId, gseaParams.sigValue);
            if(lstFeatures.size() == 0)
                app.ctls.alertInformation("GSEA Enriched Features", "We found 0 features in the Feature List.");
        }
        scatChart.setTitle("GSEA Enriched Features - " + title);
        ...
        ArrayList<ArrayList<DataGSEA.GSEAResultsData>> lstDisplayData = new ArrayList<>();
        for(ArrayList<DataGSEA.GSEAResultsData> lst : lstChartData) {
            if(maxedout)
                break;
            
            XYChart.Series<Number, String> series = new XYChart.Series<>();
            DataGSEA.GSEAResultsData ffrd = lst.get(0);
            ...
            ArrayList<DataGSEA.GSEAResultsData> lstDD = null;
            for(DataGSEA.GSEAResultsData frd : lst) {
                boolean add = false;
                if(topn) {
                ...
                if(add) {
                    totalFeatures++;
                    addSeries = true;
                    if(frd.numInCat < minx)
                        minx = frd.numInCat;
                    if(frd.numInCat > maxx)
                        maxx = frd.numInCat;
                    if(frd.source.equals("miRWalk") || frd.source.equals("REACTOME") || frd.source.equals("SIGNALP_EUK"))
                        series.getData().add(new XYChart.Data(frd.numInCat, frd.feature));
                    else
                        series.getData().add(new XYChart.Data(frd.numInCat, frd.description));
                    if(lstDD == null) {
                        lstDD = new ArrayList<>();
                ...
                if(totalFeatures >= DlgGSEAFilterFeatures.Params.MAX_SELECTIONS) {
                    maxedout = true;
                    break;
                }
            }
        ...
        if(lstSeries.isEmpty())
            app.ctls.alertInformation("GSEA Enriched Features", "There are no enriched features.");
        else if(maxedout)
            app.ctls.alertInformation("GSEA Enriched Features", "The number of selected features exceeded the limit." + 
                    "\nOnly the first " + DlgGSEAFilterFeatures.Params.MAX_SELECTIONS + " are being displayed.");
        ...
        for(XYChart.Series<Number, String> s : lstSeries) {
            ArrayList<DataGSEA.GSEAResultsData> lstSeriesData = lstDisplayData.get(idxData++);
            for(int i = 0; i < s.getData().size(); i++) {
                DataGSEA.GSEAResultsData seriesData = lstSeriesData.get(i);
...
//                double r = d / 2.0;
    ...
    private void selectFeaturesDisplayed() {
        ...
        DlgGSEAFilterFeatures dlg = new DlgGSEAFilterFeatures(project, wnd);
        DlgGSEAFilterFeatures.Params results = dlg.showAndWait(filterParams, hmFeatureIDs);
        if(results != null) {
    ...
    private void loadData() {
        lstChartData = gseaData.getGSEAEnrichedChartData(gseaParams.dataType.name(), analysisId, gseaParams.sigValue);
    }
}
