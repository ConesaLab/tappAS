 ...
 */
public class DrillDownFEAClusterNode extends DlgBase {
    static int id = 0;
    ...
    DataFEA feaData;
    ObservableList<FEAClusterNodeDrillDownData> data = null;
    
    public DrillDownFEAClusterNode(Project project, Window window) {
        super(project, window);
        toolId = ++id;
    }
    public HashMap<String, String> showAndWait(DlgFEAnalysis.Params params, String analysisId, String cluster, String node) {
        if(createToolDialog("DrillDownFEAClusterNode.fxml", "FEA Cluster Node Drill Down Data",  null)) {
            this.params = params;
            ...
            // setup dialog
            exportFilename = "tappAS_FEA_ClusterNode_"+params.name+".tsv";
            dialog.setTitle("FEA Cluster Drill Down Data for Node " + node);
            ...
            btnHelpTbl.setOnAction((event) -> { DlgHelp dlg = new DlgHelp(); dlg.show(title, "Help_DrillDown_FEAClusterNode.html", Tappas.getWindow()); });
    ...
    protected void addGeneDVItem(ContextMenu cm, TableView tbl, String panel) {
        ...
        item.setOnAction((event) -> {
            ...
            if(lstIdxs.size() != 1) {
                ...
                FEAClusterNodeDrillDownData dd = (FEAClusterNodeDrillDownData) tbl.getItems().get((tbl.getSelectionModel().getSelectedIndex()));
    ...
    private class DataService extends TaskHandler.ServiceExt {
        ...
        protected void onFailed() {
            ...
            if(e != null)
                app.logWarning("DrillDownFEAClusterNode failed - task aborted. Exception: " + e.getMessage());
            else
                app.logWarning("DrillDownFEAClusterNode - task aborted.");
            app.ctls.alertWarning("Drill Down Funtional Enrichment Analysis Cluster Node", "DrillDownFEAClusterNode - task aborted");
        }
        @Override
        protected void onSucceeded() {
            if(data != null) {
                ...
                app.ctls.alertWarning("FEA Cluster Node Drill Down Data", "Unable to load drill down data.");
        }
        @Override
                ...
                protected Void call() throws Exception {
                    try {
                            ...
                            if(st.feature.get().equals(node)) {
                                ...
                                for(int mbr = 0; mbr < bs.size(); mbr++) {
                                    if(bs.get(mbr)) {
                                        ...
                                        data.add(new FEAClusterNodeDrillDownData(member, hmTest.containsKey(member), geneDesc));
                                    }
                                }
                            }
                            idx++;
                        ...
                        if(data != null)
                            ...
                            app.logError("Unable to find node, " + node + ", in FEA results.");
                    }
                    catch (Exception e) {
    ...
    // Data Classes
    //
    static public class FEAClusterNodeDrillDownData implements Comparable<FEAClusterNodeDrillDownData> {
        public SimpleStringProperty gene;
        public SimpleStringProperty inTest;
        public SimpleStringProperty geneDescription;
        public FEAClusterNodeDrillDownData(String gene, boolean inTest, String geneDescription) {
        ...
        @Override
        public int compareTo(FEAClusterNodeDrillDownData td) {
